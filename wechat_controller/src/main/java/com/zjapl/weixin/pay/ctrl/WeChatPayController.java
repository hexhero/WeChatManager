...
import com.zjapl.weixin.pay.service.OrderService;
...
public class WeChatPayController {
	 ...
	 * @return
	 */
	@RequestMapping("/systemPay")
	@ResponseBody
	public Map<Object, Object> submitLocal(String openid, String state, HttpServletRequest request){

		state = orderService.queryOrder(state); //根据订单号查询出订单信息
		Map<String, String> paramMap = parseOrderState(state); //解析state的订单参数

		if(paramMap == null){
			HashMap<Object, Object> map = new HashMap<>();
			map.put("message", "state参数错误, 参数格式如下:<br/> state=金额*商品名称*单号 <br/> 金额单位: 分");
			return map;
		}

		SortedMap<Object, Object> payparam = prepare(openid, request, paramMap);
		payparam.put("order_num", paramMap.get("order"));//返回订单号
		return payparam;
	}
	...
	public String pay_notify(@RequestBody String value){
		...
		QueryOrderResult orderResult = WeiXinXmlUtils.xml2QueryOrderResult(value);
		if(orderResult == null){
			return "<xml>  <return_code><![CDATA[FAIL]]></return_code>  <return_msg><![CDATA[空订单信息]]></return_msg></xml>";
		}
		orderService.notifyPayResult(orderResult); //通知支付结果
		//校验金额是否正确
		String orderNum = orderResult.getOut_trade_no();
		String order = orderService.queryOrder(orderNum);
		Map<String, String> orderMap = parseOrderState(order);
		String fee = orderMap.get("fee");
		if(fee.equals(orderResult.getTotal_fee())){ //订单金额校验失败
